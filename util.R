...
calc_correlation = function(estimate.rct, V.rct, estimate.pred, return_ci=F, return_profile=F) {
  ...
  # Move data into long format (could probably do this with pivot_longer, I just wanted to make sure the order was right)
  data = tibble(est = estimate.rct, pred = estimate.pred) %>%
    mutate(row_id = row_number())
  # TODO: cleaner to pivot_longer and arrange by name
  data = bind_rows(
  ...
  out$fit = list(fit)
  out$se = if(any(is.na(fit$vvc))) NA else sqrt(fit$vvc['rho', 'rho']) #TODO this shouldn't ever really happen?
...
calc_correlation_comparison = function(estimate, V, pred1, pred2) {
  ...
  if(! any(is.na(fit$vvc))) {
    ...
    # zou = cocor(~ a + b | a + c, data = as.data.frame(tibble(a=estimate, b=pred1, c=pred2)), return.htest = T)$zou2007
    # corr_diff_se = abs(zou$conf.int[[2]] - zou$conf.int[[1]]) / (2*qnorm(0.975))
    # if(!is.na(corr_diff_se)) out = out %>% mutate(
    #   corr_diff_se = corr_diff_se,
    #   corr_diff_se_method = "zou2007"
    # )
  #}
...
save_vars = function(df_vars, filename) {
  df_vars %>%
    ...
    paste0(collapse="\n") %>%
    write_lines(str_glue("output/variables/{filename}"))
}
