...
df_predictions_enemblesize = (with_progress({
  ...
  df_model_predictions = df_llm_predictions %>% transmute(model, study, condition.name, outcome.name, y=expectation_rescaled, weight, spec_template_group, hypothesis=NA)
  # Calculate prediction condition means
  participants="all"
  
  
  ensemble_sizes = c(1,2,4,8,15,30,60,120)
  p = progressor(length(ensemble_sizes)*n_runs)
  tibble(ensemble_size = ensemble_sizes) %>% crossing(repeat_idx=1:n_runs) %>% future_pmap_dfr(.options=furrr_options(seed=T), function(ensemble_size, repeat_idx) {
    p()
    samp = load_subsample("all", repeat_idx)
    keep_spec_groups = sample(1:120, ensemble_size)
    df_condition_means_model = df_model_predictions %>%
      group_by(model, study, outcome.name, condition.name, hypothesis) %>%
        slice(keep_spec_groups) %>%
    ...
    df_predictions %>% right_join(samp) %>% mutate(ensemble_size, repeat_idx)
  })
}))
...
g
ggsave("../results/plots/ensemblesize.pdf", g, width=8, height=5)