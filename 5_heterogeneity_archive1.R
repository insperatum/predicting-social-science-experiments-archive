# 10 mins
# TODO ADD N BLACK AND NWHITE ETC
...
df_fig_subgroups = sims_subgroups %>%
  group_by(spec, model) %>%
  summarize(across(c(estimate, ci.lb, ci.ub, cor_raw, cor_raw_ci.lb, cor_raw_ci.ub), ~median(., na.rm=T))) %>% # NOTE: Do we really need na.rm=T?
  ungroup() %>%
  filter(model=="gpt-4") %>%
  mutate(
...
ggsave("output/plots/subgroups.pdf", g_subgroups, width=5.5, height=4.2)




# # Are demographic differences statistically significant
