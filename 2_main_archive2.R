...
results_by_study_outcome = run_cached("output/processed_data/megastudy_profiles.rds", \() with_progress({
  ...
  df_studies_outcomes %>% mutate(i=row_number()) %>% future_pmap_dfr(function(i, dataset, outcome, df, V, N, ...) {
  # df_studies_outcomes %>% mutate(i=row_number()) %>% pmap_dfr(function(i, dataset, outcome, df, V, N, ...) {
...
g = d_plot_all %>%
  ...
  make_archive2_plot
ggsave("output/plots/megastudies.pdf", g, width=6.3, height=6.3)
...
g_supp = d_plot_all %>%
  ...
  make_archive2_plot
g_supp
ggsave("output/plots/megastudies_supplement.pdf", g_supp, width=6.3, height=8)
