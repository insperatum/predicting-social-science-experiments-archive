...
sims_avg_2b = sims_2b %>%
  group_by(model) %>%
    summarize(across(c(estimate, ci.lb, ci.ub, cor_raw, cor_raw_ci.lb, cor_raw_ci.ub, rmse, rmse_adj), ~median(., na.rm=T))) %>%
...
# Create the figure
# TODO: Delete code for models we're not using
df_fig2b = sims_avg_2b %>%
  mutate(model_class = case_when(
    model %in% c("babbage-002", "davinci-002", "gpt-3.5-turbo", "gpt-4") ~"gpt",
    model %in% c("google/gemma-3-27b-it", "deepseek/deepseek-chat-v3-0324", "moonshotai/kimi-k2", "kimi-deepseek-gemma", "openai/gpt-oss-120b") ~ "open",
...
sims_avg_2c = sims_2c %>%
  group_by(spec, model) %>%
    summarize(across(c(estimate, se, ci.lb, ci.ub, cor_raw, cor_raw_ci.lb, cor_raw_ci.ub, rmse, rmse_adj, mse_adj_se), ~median(., na.rm=T))) %>% # NOTE: Do we really need na.rm=T?
    mutate(across(c(rmse, rmse_adj), ~.*100),
...
g
ggsave("output/plots/primary_archive.pdf", g, width=11.5, height=4.5)